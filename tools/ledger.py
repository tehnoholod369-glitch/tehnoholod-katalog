...
PRAYS_MONTAZHA = os.path.join(OTDEL_DIR, "prays-montazha.json")
...
def raboty_po_prajsu(blokov, gruppa="split", km=None):
    ...
    if not razdel.get("utverzhdeno"):
        raise SystemExit(
            ...
            % (gruppa, razdel.get("_predlozhenie", "см. prays-montazha.json в папке данных отдела")))
...
def main(argv=None):
    ...
    kj.add_argument("--montazh", type=int, metavar="БЛОКОВ",
                    help="подставить работы из prays-montazha.json в папке данных отдела "
