 ...
 *
 * ⚠️ «Хит продаж» этим скриптом НЕ ставится и ставиться не будет: для такой метки
 * нужна статистика продаж, которой у нас нет. Метка называется «Наш выбор» и
 * означает нашу рекомендацию — так же, как в каталоге (picks.json, 00_ГДЕ_ПРАВДА.md §9).
 */
(function () {
  ...
  function заполнить() {
    ...
    Promise.all([взять("index.json"), взять("picks.json")]).then(function (общее) {
      var idx = общее[0], p = общее[1];
      var skus = (p && p.skus) || [];
