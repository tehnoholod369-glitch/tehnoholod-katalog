        ...
        взять(slug + ".json").then(function (items) {
          ...
          пики.filter(function (e) { return слаг(e.getAttribute("data-pick")) === slug; })
            .forEach(function (a) {
              ...
              var сужение = a.getAttribute("data-pick-only") || изДанных || "";
              // «—» из реестра значит «на этой статье карточки быть не должно».
              // Генератор её уже не рисует, но в блоках, вставленных в Tilda
              // раньше, она осталась — снимаем здесь, чтобы не перевставлять блок.
              if (/^(-|—|нет)$/.test(сужение.trim())) {
                if (a.parentNode) a.parentNode.removeChild(a);
                return;
              }
              var только = сужение.split("|");
