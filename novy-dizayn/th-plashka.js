 ...
 * страницах читается как небрежность. По той же причине на самой Главной плашка
 * НЕ показывается — там эта фраза уже стоит в первом экране, и два одинаковых
 * сообщения в одном экране читаются как недосмотр вёрстки.
 *
...
(function () {
  ...
  /** Страница, которая уже говорит это своим текстом (первый экран Главной),
   *  второй раз не спрашивают. Ищем по самой фразе, а не по адресу страницы:
   *  уберут строчку с Главной — плашка появится там сама, без правки кода. */
  function ужеСказано() {
    var т = (document.body && document.body.innerText) || "";
    return т.indexOf("работает в тестовом режиме") >= 0;
  }

  function показать() {
    if (document.getElementById(ID) || закрыта() || ужеСказано()) return;
    стили();
  ...
  /** На Tilda текст страницы приезжает по сети (th-page.js забирает блок и
   *  отдаёт его шаблонизатору), и к 1,2 с страница бывает ещё пустой. Если
   *  показать плашку в этот момент, на Главной она встанет рядом с той же
   *  фразой, которую мы и хотели не повторять. Поэтому ждём, пока страница
   *  наполнится, но не дольше пяти секунд — на битой странице плашка нужнее.
   *
   *  16.09.2026 (T-006, раунд 3): «наполнилась» по 400 символам больше не годится —
   *  серверный SEO-раздел даёт их сразу, и при холодной загрузке плашка всплывала на
   *  Главной в каждом прогоне (замер: ~3 с поверх заглушки), пока React ещё ехал.
   *  Страница редизайна теперь сама говорит, что готова: th-page.js ставит на <html>
   *  data-th-state = loading → ready | error. Ждём ready (до 30 с); при error плашка
   *  не нужна — на экране уже сообщение с тем же WhatsApp. Старый загрузчик атрибута
   *  не ставит (из кэша браузера) — тогда прежнее правило по тексту. */
  function пуск() {
    var попыток = 0, ждалиГотовность = 0;
    (function ждать() {
      var st = document.documentElement.getAttribute("data-th-state");
      if (st === "loading" && ++ждалиГотовность < 60) { setTimeout(ждать, 500); return; }
      if (st === "loading" || st === "error") return;
      var пусто = ((document.body && document.body.innerText) || "").length < 400;
      if (st !== "ready" && пусто && ++попыток < 5) { setTimeout(ждать, 800); return; }
      показать();
    })();
  }
